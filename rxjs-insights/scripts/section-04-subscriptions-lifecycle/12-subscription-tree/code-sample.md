...
The closed property is your verification tool. You can check it at any point in your code to know whether a Subscription is active. It's false while the subscription is running, true after unsubscribe() or a natural completion. In debugging or state machines, you might check this property to guard against operations on dead subscriptions. It's a simple boolean, but it tells you the whole story of lifecycle.

In a real application, the parent Subscription would be created in a component's initialization code, and parent.unsubscribe() would be called in the component's destroy hook. Every stream the component creates gets added to the parent. When the component goes away, one line cleans up everything. Angular does this automatically with takeUntilDestroyed; React apps hand-wire it by adding subscriptions to a parent in a useEffect cleanup function. That's the pattern in practice — the tree saves you from manual bookkeeping.
